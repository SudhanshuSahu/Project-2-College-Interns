...
const createClg = async function (req, res) {
    ...
    const getClg = async function(req,res)
    {
        try{
            let data = req.query
            if(!data) return res.status(400).send({status:false , msg:"Query should not be empty"})

            let key = Object.keys(data)
            if(key.length>1) return res.status(400).send({status:false , msg:"You have to give only one query"})

            if( key[0] != "name") return res.status(400).send({status:false , msg:"You must give only college name in the query"})
            
            let clg  = await collegeModel.findOne({ name:data.name})
